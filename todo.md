...
- [ ] **Custom Ore + One Tool (Vanilla-Focused, Region-Gated)**: Add a new ore that spawns only in specific biomes, drops a raw item, and fuels both community goals and one special tool (no full armor set, no netherite replacement)
    - [x] Define biome/region gating and vein size (rarer than diamond, higher concentration in target regions)
    - [x] Define processing (smelt/blast to ingot) and limit Fortune scaling if needed
    ...
    - [x] Implement special tool: Catalyst Pickaxe (Fortune II-style drops + Haste while held)
